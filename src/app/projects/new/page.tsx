  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (images.length > 0) {
      const urls = await Promise.all(
        images.map((img, index) => uploadProjectImage(project.id, index, img.file)),
      );
      const attachRes = await fetch(`/api/projects/${project.id}/attachments`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (!attachRes.ok) {
        ...
        setError(data.error ?? "Project created but images failed to upload");
        setSaving(false);
