...
export async function POST(
  ...
  const existing = await prisma.projectCommentVote.findUnique({
    where: { userId_commentId: { userId: user.id, commentId } },
  });

  if (existing) {
    if (existing.value === value) {
      await prisma.projectCommentVote.delete({ where: { id: existing.id } });
      return NextResponse.json({ vote: null });
    }
    const updated = await prisma.projectCommentVote.update({
  ...
  const created = await prisma.projectCommentVote.create({
