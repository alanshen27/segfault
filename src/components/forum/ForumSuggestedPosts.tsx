...
export default function ForumSuggestedPosts({ posts, communityName }: ForumSuggestedPostsProps) {
  ...
  return (
    <section className="mt-8 pt-8">
      ...
      <div className="overflow-hidden divide-y divide-neutral-200 dark:divide-neutral-800/60">
        {posts.map((post) => (
          <Link
            key={post.id}
            href={`/forum/${post.id}`}
            className="flex items-start gap-4 px-4 py-3 hover:bg-neutral-100/80 dark:hover:bg-neutral-900/60 transition-colors group"
          >
            <div className="shrink-0 w-10 text-center pt-0.5">
              <span className="block text-sm font-bold text-neutral-700 dark:text-neutral-300 tabular-nums">
                {post.voteScore}
              </span>
            ...
            <div className="flex-1 min-w-0">
              ...
              <h3 className="text-sm font-semibold text-neutral-900 dark:text-neutral-100 group-hover:text-primary transition-colors line-clamp-2">
                {post.title}
              </h3>
